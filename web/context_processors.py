...
def menu(request):
    ...
    menu_commentaries = Publication.objects.filter(category=3, publish=True).order_by('-date_publish')[:2]
    menu_research_report = Publication.objects.filter(category=11, publish=True).order_by('-date_publish')[:3]
    menu_policy_brief = Publication.objects.filter(category=2, publish=True).order_by('-date_publish')[:3]
    menu_working_paper = Publication.objects.filter(category=12, publish=True).order_by('-date_publish')[:2]
