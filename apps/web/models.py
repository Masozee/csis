...
class UploadedFile(models.Model):
    ...
    def generate_unique_url(self):
        length = 6  # Length of the unique URL
        characters = string.ascii_letters + string.digits  # Use alphanumeric characters
        unique_url = ''.join(random.choice(characters) for _ in range(length))
        while UploadedFile.objects.filter(unique_url=unique_url).exists():
            unique_url = ''.join(random.choice(characters) for _ in range(length))
        return unique_url

    def get_download_url(self):
        return f'/download/{self.unique_url}'
